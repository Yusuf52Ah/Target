...
async function getEligibleSpecialist(specialistId: string) {
...
export async function createOrderAction(
  ...
  try {
    ...
    const specialist = await getEligibleSpecialist(payload.data.specialistId);
    if (!specialist || (specialist.role !== "SPECIALIST" && specialist.role !== "ADMIN")) {
      return { success: false, message: "Tanlangan foydalanuvchi mutaxassis emas yoki topilmadi." };
    }
...
export async function acceptOrderAction(orderId: string) {
  ...
  if (order.status !== "KUTILMOQDA") {
    throw new Error("Faqat kutilayotgan buyurtmani qabul qilish mumkin.");
  }
...
export async function rejectOrderAction(orderId: string) {
  ...
  if (order.status !== "KUTILMOQDA") {
    throw new Error("Faqat kutilayotgan buyurtmani rad etish mumkin.");
  }
...
export async function completeOrderAction(orderId: string) {
  ...
  if (order.status !== "QABUL_QILINDI") {
    throw new Error("Faqat qabul qilingan buyurtma yakunlanadi.");
  }
...
export async function holdEscrowAction(orderId: string, provider: "PAYME" | "CLICK") {
  ...
  if (user.role !== "ADMIN" && order.clientId !== user.id) {
    throw new Error("Escrow to'lovini faqat buyurtma mijozi tasdiqlaydi.");
  }

  if (order.status !== "KUTILMOQDA" && order.status !== "QABUL_QILINDI") {
    throw new Error("Ushbu buyurtma holatida escrow to'lovini ushlab bo'lmaydi.");
  }
