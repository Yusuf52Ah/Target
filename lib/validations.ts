...
export const searchSchema = z.object({
  ...
  message: "Maksimal narx minimal narxdan kichik bo'lmasligi kerak.",
  path: ["maxPrice"],
});
...
export const cloudinarySignSchema = z.object({
  timestamp: z.coerce.number().int().refine((value) => {
    const now = Math.floor(Date.now() / 1000);
    return Math.abs(now - value) <= 15 * 60;
  }, {
    message: "Imzo vaqti juda eski yoki noto'g'ri.",
  }),
  folder: z
    .string()
    .trim()
    .min(1, "Cloudinary papkasini kiriting.")
    .max(120, "Cloudinary papkasi juda uzun.")
    .regex(/^[a-zA-Z0-9/_-]+$/, "Cloudinary papkasi formati noto'g'ri.")
    .default("target-portfolio"),
  publicId: z.string().optional(),
  expireAt: z.coerce.date().optional().refine((value) => (value ? value.getTime() >= Date.now() : true), {
